<template>
  <div>
    <div class="card" v-if="showKontak">
      ...
      <div class="card-body">
        ...
        <ul class="list-contact table-hover" v-for="channel in channels">
          <li class="p-2 contact">
            <div class="row">
              ...
              <div class="col">
                ...
                >{{ channel.user ? channel.user.name : '-' }}</div>
        ...
        <!-- </div> -->
        <!-- <div v-else> -->
        <button class="btn btn-primary" @click="createChat">Buat Chat</button>
        <!-- </div> -->
      </div>
    </div>
    <div class="card" v-else>
      <div class="card-header">
        <button class="btn btn-sm btn-danger" @click="closeChannel()">
...
export default {
  ...
  methods: {
    getMyChanel() {
      axios.get("/chat/chanels").then(({ data }) => {
        console.log("data channe >", data);
        this.channels = data.data; //JSON.parse(data.data);
      });
    },
    createChat() {
      axios
    ...
    openChat(id) {
      this.connectChanel(id);
    },
    closeChannel() {
      this.activeChannel = null;
      this.showKontak = true;
    },
    connectChanel(id) {
      this.activeChannel = id;
      Echo.channel(`laravel_database_${this.activeChannel}`).listen(
        "MessageSent",
        ({ message }) => {
      ...
      this.getMessage();
      this.showKontak = false;
    },
    getMessage() {
      // get message
      axios
        .get(`/message/${this.activeChannel}`)
        .then(({ data }) => {
